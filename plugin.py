                        ...
                        if code in self.KP_codes :
                            ...
                            if mode == "Disarm" :
                                if not self.Actual_SS_Level =< 10 :
                                    DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=10".format(self.SS_control_idx))
                                    DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=10".format(self.KP_feedback_idxs))
                            else : # System Not ready
                                DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set%20Level&level=50".format(self.KP_feedback_idxs))
                                time.sleep(2)
                                DomoticzAPI("type=command&param=udevice&idx=IDX&nvalue=0&svalue=Waiting{}".format(self.KP_order_idxs))
