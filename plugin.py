...
<plugin key="ZZ-SSLKEYPAD" name="RONELABS - SS Lite KEYPAD Control" author="ErwanBCN" version="1.0.2" externallink="https://ronelabs.com">
    ...
    <params>
        <param field="Password" label="Codes or TagID (CSV List)" width="600px" required="true" default="1234"/>
...
class BasePlugin:
    def __init__(self):
        ...
        self.Actual_SS_FB_Level = 0
        self.Actual_KP_level = 0
        self.Actual_KP_texte = False
        self.Arming_type = 1
        self.ErrorOrNotReady = False
        self.ErrorOrNotReadyTime = datetime.now()
    ...
    def onStart(self):
        ...
        self.KP_codes = parseCSV_to_codes(Parameters.get("Password", ""))
    ...
    def onHeartbeat(self):
        ...
        if not self.Softrestartingtime + timedelta(seconds=30) <= now: #Domoticz or plugin just restarting, so wait for Alarm initilization
            Domoticz.Debug("--------------DEBUG : Plugin starting delay")
        else :
            # Plugin started ok so can run
            if self.ErrorOrNotReadyTime + timedelta(seconds = 5) <= now:
    ...
    def refresh_and_act(self):
        ...
        # checking SS Control state
        SSCheckControlAPI = DomoticzAPI("type=command&param=getdevices&rid={}".format(self.SS_control_idx))
        if SSCheckControlAPI and "result" in SSCheckControlAPI:
            for device in SSCheckControlAPI["result"]:
        ...
        if SSCheckFeedbackAPI and "result" in SSCheckFeedbackAPI:
            for device in SSCheckFeedbackAPI["result"]:
                if "LevelInt" in device:
                    Domoticz.Debug("--------------DEBUG : device: {} - {} = {}".format(device.get("idx"), device.get("Name"), device.get("LevelInt")))
                    self.Actual_SS_FB_Level = device["LevelInt"]
        ...
        if KPCheckOrderAPI and "result" in KPCheckOrderAPI:
            for device in KPCheckOrderAPI["result"]:
                ...
                if idx in self.KP_order_idxs and "Data" in device:
                    ...
                    # 2) Check code or TagID
                    # try int, else string
                    try:
                        code_val = int(code_str)
                    except:
                        code_val = code_str  # tag RFID string
                    ...
                    if mode == "Disarm":
                        if code_ok:
                            if system_armed:
                                ...
                                Domoticz.Log("--- Keypad Code or RFID OK pour 'Disarm'---")
                            else:
                                # déjà désarmé → feedback d'erreur (optionnel) puis reset
                                feedback_error()
                                Domoticz.Log("--- !!! Code OK or RFID for 'Disarm' but order Error : 'Already Disarmed' !!!")
                            reset_text_field()
                        else:
                            Domoticz.Log("--- !!! Keypad CODE or RFID ERROR (Disarm) !!! ---")
                            feedback_error()
                            reset_text_field()
                        ...
                        if (code_ok or can_arm_without_code) and ready_to_arm:
                            ...
                            if can_arm_without_code :
                                Domoticz.Log("--- Keypad Order OK Without Code for 'Arm'---")
                            else :
                                Domoticz.Log("--- Keypad Code or RFID OK for 'Arm'---")
                            reset_text_field()

                        else:
                            # Conditions non remplies → erreur
                            Domoticz.Log(f"--- !!! ARMING ORDER ERROR ({mode}) : "
                                         f"--- !!! CodeOK/Rfid= {code_ok}, AllowNoCode= {can_arm_without_code}, ReadyToArm= {ready_to_arm}, SS Armed= {system_armed}, SS Level= {self.Actual_SS_Level}")
                            feedback_error()
                            reset_text_field()
...
def parseCSV_to_codes(s):
    codes = []
    for x in s.split(','):
        x = x.strip()
        if not x:
            continue
        # si c'est un nombre pur → int
        if x.isdigit():
            codes.append(int(x))
        else:
            # sinon garder en string (tags RFID etc.)
            codes.append(x)
    return codes
