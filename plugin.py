...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.Softrestartingtime + timedelta(seconds=65) <= now: #Domoticz or plugin just restarting, so wait for Alarm initilization
        ...
        if SSCheckFeedbackAPI and "result" in SSCheckFeedbackAPI:
            for device in SSCheckFeedbackAPI["result"]:
                if "LevelInt" in device:
                    Domoticz.Debug(
                        "device: {}-{} = {}".format(device.get("idx"), device.get("Name"), device.get("LevelInt")))
            ...
            for device in KPCheckOrderAPI["result"]:
                ...
                if idx in self.KP_order_idxs:
                    if "Data" in device:
                        ...
                        # Les codes sont stockés en int → on convertit
                        try:
                            code_val = int(code_str)
                        except ValueError:
                            code_val = -1  # invalide

                        if code_val in self.KP_codes:
                            Domoticz.Log(f"Keypad Code OK pour l’ordre: '{mode}'")

                            # Système armé ? (niveau ≠ 0 → armé/partiellement armé)
                            system_armed = (self.Actual_SS_Level >= 20)


                            if mode == "Disarm":
                                if system_armed:
                                    dz_switchlevel(self.SS_control_idx, 10)
                                else:
                                    # Déjà désarmé → feedbackError + reset
                                    for f_idx in self.KP_feedback_idxs:
                                        dz_switchlevel(f_idx, 50)
                                    time.sleep(1)
                                    DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Waiting".format(idx))

                            elif mode in ("ArmHome", "ArmNight", "ArmAllZones"):
                                # Exemple simple : on met un niveau d’armement générique 50 (à adapter)
                                dz_switchlevel(self.SS_control_idx, 50)
                                for f_idx in self.KP_feedback_idxs:
                                    dz_switchlevel(f_idx, 50)
                                time.sleep(1)
                                DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Waiting".format(idx))
                            ...
                            # BAD CODE
                            Domoticz.Log("Keypad CODE ERROR")
                            for f_idx in self.KP_feedback_idxs:
                                dz_switchlevel(f_idx, 50)
                            time.sleep(1)
                            DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Waiting".format(idx))
