...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if not self.Softrestartingtime + timedelta(seconds=30) <= now: #Domoticz or plugin just restarting, so wait for Alarm initilization
            Domoticz.Debug("--------------DEBUG : Plugin starting delay")# refresh values and act
            self.refresh_and_act()
        else :
            # Plugin started ok so can run
            if self.ErrorOrNotReadyTime + timedelta(secondes = 5) <= now:
                    ...
                    if mode == "Disarm":
                        if code_ok:
                            if system_armed:
                                ...
                                feedback_error()
                                Domoticz.Log("--- !!! Code OK pour 'Disarm' but order Error : 'Already Disarmed' !!!")
                        reset_text_field()
                        else:
