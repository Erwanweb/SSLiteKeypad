...
<plugin key="ZZ-SSLKEYPAD" name="RONELABS - SS Lite KEYPAD Control" author="ErwanBCN" version="1.0.2" externallink="https://ronelabs.com">
    ...
    <params>
        ...
        <param field="Mode1" label="SS Lite widget idx : Control, Feedback (CSV List of idx)" width="200px" required="true" default=""/>
        ...
        <param field="Mode5" label="Arming type" width="300px">
            <options>
            ...
            for device in KPCheckOrderAPI["result"]:
                ...
                if "Data" in device:
                    ...
                    if not self.Actual_KP_texte == "Waiting"
                        ...
                        Domoticz.Debug(f"Keypad order mode '{mode}'.")
                        Domoticz.Debug(f"Keypad order mode '{code}'.")
                        if code in self.KP_codes :
                            Domoticz.Log(f"Keypad Code OK for order : '{mode}'.")
                            if not self.Actual_SS_Level == 0:
                                if mode == "Disarm" :
                                    if not self.Actual_SS_Level =< 10 :
                                        dz_switchlevel(self.SS_control_idx, 10)
                                    else :
                                        dz_switchlevel(self.KP_feedback_idxs, 50)
                                        time.sleep(1)
                                        DomoticzAPI("type=command&param=udevice&idx={}&nvalue=0&svalue=Waiting".format(self.KP_order_idxs))
