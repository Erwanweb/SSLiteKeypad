...
class BasePlugin:
    def __init__(self):
        ...
        self.Arming_type = 1
        self.ARM_THRESHOLD = 20  # à partir de 20 = armé/partiellement armé
    ...
    def onStart(self):
        ...
        self.Arming_type = int(Parameters.get("Mode5", "1"))  # 1 = With Code (défaut), 0 = Without Code
        ...
        if KPCheckOrderAPI and "result" in KPCheckOrderAPI:
            for device in KPCheckOrderAPI["result"]:
                ...
                if idx in self.KP_order_idxs and "Data" in device:
                    ...
                    #    - Seuil armement : >= ARM_THRESHOLD
                    #    - Désarmement : code toujours requis
                    #    - Armement : si Arming_type == 0, code non requis pour armer
                    system_armed = (self.Actual_SS_Level >= self.ARM_THRESHOLD)
                    allow_arm_without_code = (self.Arming_type == 0)
                    ...
                    if mode == "Disarm":
                        ...
                        if code_ok or allow_arm_without_code:
                            if not system_armed:
                                if mode == "ArmHome":
                                    ...
                                    Domoticz.Error(f"Mode inconnu (armement): '{mode}'")
                                    feedback_error()
                                reset_text_field()
                            else:
                                # déjà armé → feedback d'erreur + reset
                                feedback_error()
                                reset_text_field()
                            ...
                            code_ok = (code_val in self.KP_codes)
                            # Nouvelle condition : on ne peut armer QUE si l'état actuel = 10 Disarmed
                            ready_to_arm = (self.Actual_SS_Level == 10)
                            ...
                            if (code_ok or can_arm_without_code) and ready_to_arm:
                                ...
                                Domoticz.Log(f"--- ORDER ERROR ({mode}) : "
                                             f"--- CodeOK= {code_ok}, AllowNoCode= {can_arm_without_code}, ReadyToArm= {ready_to_arm}, Already Armed= {system_armed}")
                                feedback_error()
                                time.sleep(1)
                                reset_text_field()
